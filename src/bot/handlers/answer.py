...
from bot.db.queries.answers import count_pending_answers, create_answer
...
async def on_answer(message: types.Message) -> None:
    ...
    pending_count = await count_pending_answers(user["id"])
    await message.answer(format_answer_response(user_prob, pending_count), parse_mode="HTML")
